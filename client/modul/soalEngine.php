...
$(document).ready(function(){
	...
	$("#endButton").click(function(){
		...
		$.ajax({
            type: "GET",
            url: "../data/data_soal_cek_sudah.php",
            data:{
                'idUjian' : idUjian,
                'nopes' : nopes
            },
...
function berakhir(){
	alert('Waktu Pengerjaan Ujian Ini Telah Berakhir');
...
//cek apakah mode server time ?
$sqlMode= mysqli_query($con,"select * from tbl_ujian where id_ujian='".$_GET['idUjian']."'");
$resMode = mysqli_fetch_array($sqlMode);

//set minuted untuk inisialisasi sisa menit
echo "<input type='hidden' id='minuted' value='cekcek'>";

if($rescekwaktudistatus['status'] != 1){
	if($jmlcekwaktudistatus > 0 && $rescekwaktudistatus['sisa_waktu'] > 0 && $resMode['time_mode'] != 1){
		...
		if($resTimer['time_mode'] == 1){
			date_default_timezone_set('Asia/Jakarta');
			$detik = (time() - $resTimer['time_set']);
			$menit = floor($detik/60);
			$jatah = $resTimer['lama_ujian'] - $menit;
			if($jatah > 0){
				echo "<input type='hidden' id='timed' value='".$jatah."'>";
			}else{
				echo '<script type="text/javascript"> berakhir(); </script>';
			}
		}else{
...
<div class="bloker3">	
	<div class="isiBloker">
		<img src="../images/warning.png"><br><br>
		Anda belum menjawab semua pertanyaan !<br><br>
		<button id="btnLanjutkan" class="buttonOrange">LANJUTKAN</button>
	</div>
</div>
